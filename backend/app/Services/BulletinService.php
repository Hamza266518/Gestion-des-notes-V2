...
{
    const WEIGHT_CC = 0.3666;
    const WEIGHT_THEORIQUE = 0.2666;
    const WEIGHT_PRATIQUE = 0.3666;
    ...
    public function calculateBulletin(int $etudiantId, int $anneeAcademiqueId, ?int $semestre = null): array
    {
        ...
        $unites = Unite::where('filiere_id', $filiereId)
            ...
            ->with('sequences.controles.notes.etudiant')
            ->orderBy('ordre')
            ->get();
        ...
        foreach ($unites as $unite) {
            ...
            $uniteTheo = $this->moyenneTypeForUnite($etudiantId, $unite, 'theorique');
            $unitePra = $this->moyenneTypeForUnite($etudiantId, $unite, 'pratique');
            ...
            foreach ($unite->sequences as $seq) {
                ...
                $sequencesData[] = [
                    ...
                    'moyenneSeq' => $this->moyenneSequence($etudiantId, $seq),
                ];
            }

            $moyenneUnite = $this->moyenneUnite($etudiantId, $unite);
    ...
    private function moyenneSequence(int $etudiantId, $sequence): ?float
    {
        $notes = [];

        foreach ($sequence->controles as $ctrl) {
            $note = $ctrl->notes->firstWhere('etudiant_id', $etudiantId);
            if ($note && $note->valeur !== null) {
                $notes[] = (float) $note->valeur;
            }
    ...
    private function moyenneUnite(int $etudiantId, Unite $unite): ?float
    {
        $totalNote = 0;
        $totalCoef = 0;

        foreach ($unite->sequences as $seq) {
            $moy = $this->moyenneSequence($etudiantId, $seq);
            if ($moy !== null) {
                $totalNote += $moy * $seq->coefficient;
                $totalCoef += $seq->coefficient;
            }
        }
